    ...
    class PucReadmeParser
    {
        ...
        function user_sanitize($text, $strict = \false)
        {
            ...
            if (\function_exists('MatthiasWeb\\RealMediaLibrary\\Vendor\\user_sanitize')) {
                // bbPress native
                return \MatthiasWeb\RealMediaLibrary\Vendor\user_sanitize($text, $strict);
            }
            if ($strict) {
        ...
        function filter_text($text, $markdown = \false)
        {
            ...
            if ($markdown) {
                ...
                $instance = \MatthiasWeb\RealMediaLibrary\Vendor\Parsedown::instance();
        ...
        function encodeit($matches)
        {
            if (\function_exists('MatthiasWeb\\RealMediaLibrary\\Vendor\\encodeit')) {
                // bbPress native
                return \MatthiasWeb\RealMediaLibrary\Vendor\encodeit($matches);
            }
            $text = \trim($matches[2]);
        ...
        function decodeit($matches)
        {
            if (\function_exists('MatthiasWeb\\RealMediaLibrary\\Vendor\\decodeit')) {
                // bbPress native
                return \MatthiasWeb\RealMediaLibrary\Vendor\decodeit($matches);
            }
            $text = $matches[2];
