...
class ImportController {
	...
	public function detect_import() {
		...
		foreach ( $this->plugins_to_import as $key => $plugin ) {
			...
			$counter = $has_import ? -1 : $this->get_folder_counter( $key );
			if ( \intval( $counter ) > 0 || -1 === $counter ) {
				$this->plugins_to_import[ $key ]['folder_counter'] = \intval( $counter );
	...
	public function get_folder_counter( $prefix ) {
		global $wpdb;
		if ( in_array( $prefix, array( 'EML', 'HP', 'Folders', 'WPMF', 'MLA' ), true ) ) {
