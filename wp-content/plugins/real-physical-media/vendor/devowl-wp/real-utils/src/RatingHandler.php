...
class RatingHandler {
    ...
    public function dismiss($slug, $force) {
        ...
        $initiator = \DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\Core::getInstance()->getInitiator($slug);
        if ($initiator === null) {
            return \false;
        }
        ...
        if (
            \DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\TransientHandler::set(
                $initiator,
                \DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_RATING,
                $ts
            )
        ) {
            return $ts;
    ...
    public function getCanBeRated() {
        ...
        // @codeCoverageIgnoreEnd
        $cache = [];
        foreach (\DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\Core::getInstance()->getInitiators() as $initiator) {
            $ts = \DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\TransientHandler::get(
                $initiator,
                \DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_RATING
            );
            // Initialize default
    ...
    public static function instance() {
        return new \DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\RatingHandler();
    }
}
