...
use DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\cross\AbstractCrossSelling;
use WP_Error;
use WP_REST_Request;
...
' .
    ...
    public function routeCrossDismiss($request) {
        ...
        $impl = \DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\Core::getInstance()->getCrossSelling($slug);
        if ($impl === null) {
            return new \WP_Error(
                'rest_error',
    ...
    public function routeRatingDismiss($request) {
        $slug = $request->get_param('slug');
        $force = $request->get_param('force');
        $result = \DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\Core::getInstance()
            ->getRatingHandler()
            ->dismiss($slug, $force);
        // Currently, ignore errors
        return new \WP_REST_Response(['result' => $result]);
    ...
    public static function instance() {
        return new \DevOwl\RealPhysicalMedia\Vendor\DevOwl\RealUtils\Service();
    }
}
